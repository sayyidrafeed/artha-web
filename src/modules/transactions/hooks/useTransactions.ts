...
export function useTransactions(filters: TransactionFilter): UseTransactionsResult {
  const { data, isLoading, error } = useQuery<TransactionsResponse>({
    ...
    queryFn: async () => api.get<TransactionsResponse>("/transactions", filters),
    staleTime: 30 * 1000,
  });
  ...
  const { mutate, mutateAsync, isPending, error } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.dashboard.summary(new Date().getFullYear()),
      });
    },
  });
...
interface UseUpdateTransactionResult {
  ...
  mutateAsync: (params: { id: string; data: UpdateTransactionInput }) => Promise<Transaction>;
  isPending: boolean;
  error: Error | null;
}
